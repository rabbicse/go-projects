...
func getOSIcon(platform string) string {
	...
	case "ubuntu":
		return "fa-ubuntu"
	default:
		return "fa-desktop" // default/fallback icon
	}
}

func generatePlatformRow(platform string) string {
	iconClass := getOSIcon(platform)
	return fmt.Sprintf("<tr><td>Platform:</td><td> <i class='fa fa-brands %s'></i> %s</td></tr>", iconClass, platform)
}
...
func GetSystemSection() (string, error) {
	...
	html = html + generatePlatformRow(hostStat.Platform)
